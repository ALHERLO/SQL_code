...
'WINDOW FUNCTIONS 👉🏽 a window function performs a calculation across a set of 
table rows that are somehow related to the current row'

'1. This time, create a running total of standard_amt_usd (in the orders table) 
...
SUM(standard_amt_usd) OVER (ORDER BY occurred_at) AS running_total 👉🏽 order by creates an accumulated sum
FROM orders

'2. create a running total of standard_amt_usd (in the orders table) over order 
